...
- 支持突发情况处理：自然语言或 API/CLI/MCP 登记请假缺席（带日期区间，到期自动失效），排班结果由 Python 硬校验并确定性修复（剔除缺席、缺额补齐、超额裁剪）
- 支持用户备注（如“周三大扫除”）注入后续排班提示词，带过期日期、不会被运行结果覆盖
- 支持单日人数覆盖（大扫除等临时加派），按日期/区域生效并参与校验
