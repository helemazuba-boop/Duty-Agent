...
if %ROBOCOPY_EXIT% leq 7 (
    set "COPIED=true"
    echo Files deployed and stale files removed.
) else (
    set /a RETRY_COUNT+=1
    if !RETRY_COUNT! lss %MAX_RETRIES% (
        echo [WARNING] File sync failed. Retrying in 1 second... (!RETRY_COUNT!/%MAX_RETRIES%)
...
if "%COPIED%"=="false" (
    echo [ERROR] Failed to sync files after %MAX_RETRIES% retries. Robocopy exit code: %ROBOCOPY_EXIT%
    exit /b 1
)
