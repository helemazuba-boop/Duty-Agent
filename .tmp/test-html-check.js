    ...
    async function loadConfigFromBackend(reason) {
      if (!hasBackendTransport) {
        setAutoApplyStatus('err', '当前页面未连接到 FastAPI。');
        return false;
      }
      const endpoint = `${window.location.origin}/api/v1/config`;
      try {
        log('INFO', '开始从 FastAPI 加载正式配置。', { reason: reason || 'manual', endpoint });
        ...
        persistLocalDebugPrefs(`load_config:${reason || 'manual'}`, { quiet: true });
        renderConfig();
        setBackendConfigDirtyState(false, '正式配置已从 FastAPI 重载。');
        log('OK', 'FastAPI 正式配置加载成功。', {
          reason: reason || 'manual',
        ...
        const message = e instanceof Error ? e.message : String(e);
        setAutoApplyStatus('err', `正式配置加载失败：${message}`);
        log('ERROR', 'FastAPI 正式配置加载失败。', { reason: reason || 'manual', message, endpoint });
        return false;
      }
    }
    ...
    function buildBackendConfigPatch() {
      ...
      const patch = {
        expected_version: lastLoadedBackendConfig.version
      };
    ...
    async function saveConfigToBackend(reason) {
      ...
      try {
        ...
        const reloaded = await loadConfigFromBackend('post_save_verify');
        if (!reloaded) {
          throw new Error('正式配置保存后校验失败，请重新加载确认。');
        }
        persistLocalDebugPrefs(`save_config:${reason || 'manual'}`, { quiet: true });
        setBackendConfigDirtyState(false, `正式配置已保存并校验通过（版本 ${store.config.version}）。`);
        log('OK', 'FastAPI 正式配置保存成功。', {
          reason: reason || 'manual',
          version: store.config.version,
          selected_plan_id: store.config.selected_plan_id
        });
        ...
        if (message.includes('Config version mismatch')) {
          await loadConfigFromBackend('version_conflict');
          setAutoApplyStatus('err', '保存失败：配置版本已变化，已自动重载最新配置。');
        } else {
          setAutoApplyStatus('err', `正式配置保存失败：${message}`);
        }
        log('ERROR', 'FastAPI 正式配置保存失败。', { reason: reason || 'manual', message, endpoint, expected_version: patch.expected_version });
        return false;
      } finally {
        updateBackendConfigUiState();
      }
    ...
    async function loadSnapshotFromBackend(reason) {
      ...
      try {
        ...
        setBackendConfigDirtyState(false, '正式配置与快照已从 FastAPI 同步。');
        selectedRosterId = null;
        renderAll();
    ...
    function importSnapshot() {
      ...
      try {
        ...
        setBackendConfigDirtyState(true, '快照已导入，正式配置尚未保存到 FastAPI。');
    ...
    function resetAll() {
      ...
      setBackendConfigDirtyState(true, '已重置为默认测试数据，正式配置尚未保存到 FastAPI。');
    ...
    function bindConfigAutoApply() {
      ...
      for (const id of backendFieldIds) {
        ...
        if (tag === 'select') {
          el.addEventListener('change', () => {
            syncConfigFromForm();
            markBackendConfigDirty(`change:${id}`);
          });
          continue;
        }
        if (tag === 'textarea') {
          el.addEventListener('blur', () => {
            syncConfigFromForm();
            markBackendConfigDirty(`blur:${id}`);
          });
          el.addEventListener('keydown', (event) => {
            if (event.key === 'Enter' && (event.ctrlKey || event.metaKey)) {
              event.preventDefault();
              syncConfigFromForm();
              markBackendConfigDirty(`hotkey:${id}`);
            }
          });
          continue;
        }
      ...
      $('cfgPlan').addEventListener('change', () => {
        ...
        renderConfig();
        markBackendConfigDirty('change:cfgPlan');
      });
    ...
    function bind() {
      ...
      $('saveConfigBtn').addEventListener('click', () => { void saveConfigToBackend('manual_save'); });
      $('reloadConfigBtn').addEventListener('click', () => { void loadConfigFromBackend('manual_reload'); });
      ...
      $('loadBtn').addEventListener('click', () => {
        ...
        if (runtimeMode === 'backend') {
          void loadSnapshotFromBackend('manual_refresh');
        } else {
          renderAll();
    ...
    if (hasBackendTransport) {
      void loadSnapshotFromBackend('startup');
    }
